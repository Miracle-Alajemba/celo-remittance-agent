...
const orchestrator_1 = require("../src/blockchain/agent/orchestrator");
...
╚════════════════════════════════════════════════════════════════╝
  `);
    ...
    const agent = new orchestrator_1.AgentOrchestrator("fresh_user_123", "0x0000000000000000000000000000000000000000");
    ...
    const walletResponse = await agent.processMessage(`My wallet address is ${testWallet}`);
    ...
    if (walletResponse.suggestedActions) {
        console.log(`   💡 Suggested Actions: ${walletResponse.suggestedActions.join(", ")}\n`);
    }
    ...
    const transferResponse = await agent.processMessage("Send $50 to my mom in the Philippines");
    ...
    if (transferResponse.suggestedActions) {
        console.log(`   💡 Suggested Actions: ${transferResponse.suggestedActions.join(", ")}\n`);
    }
    // ===== STEP 5: Test in Spanish =====
    console.log(`\n🇪🇸 STEP 5: User sends message in Spanish\n`);
    const spanishAgent = new orchestrator_1.AgentOrchestrator("spanish_user", "0x0000000000000000000000000000000000000000");
