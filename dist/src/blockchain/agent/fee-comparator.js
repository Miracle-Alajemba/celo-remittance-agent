...
function compareFees(amount, sendCurrency, receiveCountry) {
    if (amount <= 0) {
        throw new Error("Amount must be greater than 0");
    }
    const receiveCurrency = getReceiveCurrency(receiveCountry);
    const ratePair = `${sendCurrency}-${receiveCurrency}`;
    const midMarketRate = (0, rates_1.getRate)(sendCurrency, receiveCurrency);
    if (!midMarketRate) {
...
function formatFeeComparison(comparison, lang = "en") {
    const { corridor, celoFees, traditionalProviders, sendAmount, sendCurrency, receiveCurrency, } = comparison;
    ...
    for (const p of traditionalProviders) {
        ...
        output += `Rate: 1 ${sendCurrency} = ${p.exchangeRate.toFixed(2)} ${receiveCurrency}\n`;
