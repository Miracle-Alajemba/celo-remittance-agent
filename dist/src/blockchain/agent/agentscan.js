...
exports.getAgentScanner = getAgentScanner;
...
class AgentScanner {
     ...
     */
    recordTransactionTrace(agentAddress, trace) {
     ...
     */
    getActivityLog(agentAddress, limit = 100) {
     ...
     */
    getActivityByType(agentAddress, type) {
        const activities = this.activities.get(agentAddress) || [];
     ...
     * Get agent analytics
     */
    getAnalytics(agentAddress) {
        return this.analytics.get(agentAddress) || null;
    }
    /**
     * Update agent analytics
     */
    updateAnalytics(agentAddress) {
        const activities = this.activities.get(agentAddress) || [];
        ...
        const avgSize = activities.length > 0
            ? (activities
                .filter(a => a.value)
                .reduce((sum, a) => sum + parseFloat(a.value || '0'), 0) /
        ...
        const trustScore = Math.min(100, Math.floor((successful / activities.length) * 100 - (failed / activities.length) * 10));
        const analytics = {
            agentAddress,
     ...
     */
    getTransactionTraces(agentAddress) {
        return this.transactions.get(agentAddress) || [];
    }
    /**
     * Get agent metrics
     */
    getMetrics(agentAddress) {
        ...
        const successRate = (analytics.successfulTransactions / analytics.totalTransactions) * 100;
        // Calculate gas efficiency (mock calculation)
        const totalGasUsed = this.getTransactionTraces(agentAddress).reduce((sum, t) => sum + (parseInt(t.gasUsed || '0') || 0), 0);
        const gasEfficiency = activities.length > 0 ? 100 - (totalGasUsed / (activities.length * 21000)) * 100 : 0;
     ...
     */
    async monitorAgentActivity(agentAddress, onActivity) {
     ...
     */
    getAgentsByTrustScore(min, max) {
        return Array.from(this.analytics.values()).filter(a => a.trustScore >= min && a.trustScore <= max);
    }
    /**
     * Generate agent report
     */
    generateReport(agentAddress) {
        const analytics = this.getAnalytics(agentAddress);
        const metrics = this.getMetrics(agentAddress);
    ...
    generateRecommendations(analytics, metrics) {
        ...
        if (metrics.successRate < 80) {
            recommendations.push('Success rate is below 80%. Review failed transactions and improve error handling.');
        }
        ...
        if (analytics.trustScore < 50) {
            recommendations.push('Trust score is low. Ensure compliance and successful transaction completion.');
        }
        if (metrics.responseTime > 10000) {
            recommendations.push('Response time is high. Consider optimizing blockchain interactions.');
        }
     ...
     */
    exportActivityData(agentAddress, format = 'json') {
...
// Singleton instance
let agentScanner = null;
function getAgentScanner() {
    if (!agentScanner) {
