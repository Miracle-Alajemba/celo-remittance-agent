...
class ConversationMemory {
    ...
    async init() {
        if (!this.userId || !(0, connection_1.isDbConnected)())
            return;
        try {
            const history = await (0, services_1.getConversationHistory)(this.userId, 50);
    ...
    addMessage(role, content, intent, metadata) {
        ...
        if (this.userId && (0, connection_1.isDbConnected)()) {
            void (0, services_1.createConversationMessage)({
                userId: this.userId,
                role,
                content,
