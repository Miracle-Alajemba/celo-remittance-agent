...
export interface AgentResponse {
  message: string;
  type: 'text' | 'transfer_preview' | 'fee_comparison' | 'history' | 'receipt' | 'schedule' | 'error' | 'help';
  data?: any;
...
export class AgentOrchestrator {
  ...
  async processMessage(userMessage: string): Promise<AgentResponse> {
    ...
    try {
      ...
      const contextStr = conversationHistory
        ...
        .join('\n');
      const llmResult = await enhanceIntentWithLLM(userMessage, intent, contextStr);
  ...
  private async handleSendIntent(intent: RemittanceIntent): Promise<AgentResponse> {
    ...
    // Get fee comparison
    const comparison = compareFees(amount, sourceCurrency, intent.recipientCountry || 'PH');
  ...
  private async handleSchedule(intent: RemittanceIntent): Promise<AgentResponse> {
    ...
    if (!intent.amount) {
      ...
      const list = schedules.map(s => formatScheduledTransfer(s, lang)).join('\n\n');
